...
/**
 * @callback CursorEncodingFunction
 * @param {DynamoDBParams} cursor
 *
 * @returns {string}
 */

/**
 * @callback CursorDecodingFunction
 * @param {string} encodedCursor
 *
 * @returns {DynamoDBParams}
 */
 ...
 * @param {CursorEncodingFunction} [encodingFunction]
 * @param {boolean} enableBackNavigation
 *
 * @returns {string | undefined}
 */
const encodeCursor = (params, lastEvaluatedKey, result, encodingFunction, enableBackNavigation = true) => {
  if (
    !result.Items
    || !result.Items.length
  ...
  const cursor = {
    ...
    previousKeys: enableBackNavigation && lastEvaluatedKey ? [...(params.previousKeys || []), lastKey] : params.previousKeys,
    back: false,
  };
 ...
 * @returns {string | undefined}
 */
const encodeBackCursor = (params, encodingFunction) => {
...
const paginatorFunctionFactory = () => ({
   ...
   * @param {number} limit
   * @param {DynamoDBResult} result
   * @param {CursorEncodingFunction} [cursorEncodingFunction]
   * @param {boolean} enableBackNavigation
   *
   ...
   * @template T
   */
  getPaginatedResult: (params, limit, result, cursorEncodingFunction, enableBackNavigation = true) => ({
    data: result.Items,
    meta: {
      limit,
      cursor: encodeCursor(params, result.LastEvaluatedKey, result, cursorEncodingFunction, enableBackNavigation),
      backCursor: enableBackNavigation ? encodeBackCursor(params, cursorEncodingFunction) : undefined,
      hasMoreData:
        params.back
   ...
   * @param {CursorDecodingFunction} [decodingFunction]
   *
   * @returns {DynamoDBParams | undefined}
   */
  decodeCursor: (encodedCursor, decodingFunction) => {
