 ...
 * @property {any[]} previousKeys Previous used ExclusiveStartKeys to navigate back to
 * @property {boolean} back To indicate wether it's a cursor used for forward or backwards navigation
 */
 ...
 */
const encodeCursor = (params, lastEvaluatedKey, result, encodingFunction) => {
  if (
    !result.Items
    || !result.Items.length
...
const paginatorFunctionFactory = () => ({
  ...
  getPaginatedResult: (params, limit, result, cursorEncodingFunction) => ({
    ...
    meta: {
      limit,
      cursor: encodeCursor(params, result.LastEvaluatedKey, result, cursorEncodingFunction),
      backCursor: encodeBackCursor(params, cursorEncodingFunction),
      hasMoreData:
        params.back
